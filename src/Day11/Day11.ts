...
const monkeys = monkeyArr.map((x) => parseMonkey(x))

let round = 0

while (round !== 10000) {
  monkeys.map((monkey, monkeyId) => {
    monkey.items.map((item, itemId) => {
      const op2 =
        monkey.operation[1] === 'old' ? item : parseInt(monkey.operation[1])
      ...
      condition
        ? monkeys[monkey.onTrue].items.push(newValue)
        : monkeys[monkey.onFalse].items.push(newValue)
